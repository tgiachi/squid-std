...
/// <summary>
/// A min-priority queue backed by a binary heap, with O(1) <see cref="Contains" />. Reach for this when raw
/// throughput matters most (tight game-loop or pathfinding hot paths) and equal-priority items don't need a
/// deterministic dequeue order - use <see cref="StablePriorityQueue{T}" /> instead when they do.
/// Unlike System.Collections.Generic.PriorityQueue, supports UpdatePriority, O(1) Contains and efficient Remove.
/// </summary>
/// <typeparam name="T">The item type stored in the queue. Must extend <see cref="FastPriorityQueueNode" />.</typeparam>
    ...
    where T : FastPriorityQueueNode
{
    ...
    /// <summary>
    /// Enqueues a node to the priority queue. Lower values are placed in front. Ties are broken arbitrarily.
    /// If the queue is full, the node is already enqueued, or the node belongs to another queue (without a
    /// preceding <see cref="ResetNode" />), the result is undefined.
    /// O(log n)
    /// </summary>
    ...
    /// <summary>
    /// Returns true if 'higher' has higher priority than 'lower', false otherwise. Note that calling
    /// <c>HasHigherPriority(node, node)</c> (both arguments the same node) returns false.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private bool HasHigherPriority(T higher, T lower) => higher.Priority < lower.Priority;

    /// <summary>
    /// Returns true if 'higher' has higher-or-equal priority than 'lower', false otherwise. Note that calling
    /// <c>HasHigherOrEqualPriority(node, node)</c> (both arguments the same node) returns true.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private bool HasHigherOrEqualPriority(T higher, T lower) => higher.Priority <= lower.Priority;

    /// <summary>
    /// Removes the head of the queue and returns it. If the queue is empty, the result is undefined.
    /// O(log n)
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    public T Dequeue()
    ...
    /// this on the original queue before re-using the node elsewhere. If the node is currently in the queue or
    /// belongs to another queue, the result is undefined.
    /// </summary>

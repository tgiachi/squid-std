...
/// <summary>
/// A copy of <see cref="FastPriorityQueue{T}" /> that is also stable - nodes enqueued with the same priority are
/// always dequeued in insertion (FIFO) order. Reach for this over <see cref="FastPriorityQueue{T}" /> in
/// schedulers and simulations where two events firing at the same tick must run in a deterministic order.
/// Unlike System.Collections.Generic.PriorityQueue, supports UpdatePriority, O(1) Contains and efficient Remove.
/// </summary>
/// <typeparam name="T">The item type stored in the queue. Must extend <see cref="StablePriorityQueueNode" />.</typeparam>
    ...
    where T : StablePriorityQueueNode
{
    ...
    /// <summary>
    /// Returns true if 'higher' has higher priority than 'lower' - by priority value, or by insertion order when
    /// priorities are equal - false otherwise.
    /// </summary>
    [MethodImpl(MethodImplOptions.AggressiveInlining)]

...
public interface IPgpService
{
    ...
    Task<string> EncryptAndSignForAsync(string recipientIdentity, byte[] data, string signerIdentity, string signerPassphrase, CancellationToken cancellationToken = default);
    ...
    Task<PgpDecryptionResult> DecryptAndVerifyAsync(string armored, string passphrase, CancellationToken cancellationToken = default);

    /// <summary>Produces an armored signed message that embeds <paramref name="data" />.</summary>
    Task<string> SignAsync(byte[] data, string signerIdentity, string passphrase, CancellationToken cancellationToken = default);
